...
func migrationCheck(repo string) (map[string]any, error) {
	...
	specFiles, err := migrationKnownFiles(specRoot)
	if err != nil {
		return nil, err
	}
	...
	for _, pair := range []struct {
		...
		files, walkErr := migrationKnownFiles(pair.Source)
		if walkErr != nil {
			return nil, walkErr
		}
	...
	designFiles, err := migrationDesignFiles(filepath.Join(root, "docs", "design"))
	if err != nil {
		return nil, err
	}
	...
	directFiles, err := migrationKnownFiles(directRoot)
	if err != nil {
		return nil, err
	}
...
func migrationApply(repo string) (map[string]any, error) {
	...
	workspace := newMethodWorkspace(root)
	type copyPair struct{ source, target string }
	copied := make([]copyPair, 0, len(entries))
	removed := make([]copyPair, 0, len(entries))
	rollback := func() {
		for index := len(removed) - 1; index >= 0; index-- {
			pair := removed[index]
			if _, sourceErr := os.Lstat(pair.source); os.IsNotExist(sourceErr) {
				_ = os.MkdirAll(filepath.Dir(pair.source), 0o755)
				_ = copyMigrationFile(pair.target, pair.source)
			}
		}
	...
	for _, entry := range entries {
		...
		if readErr != nil || sha256Bytes(content) != entry.SHA256 {
			rollback()
			return nil, workflowError("MIGRATION_REQUIRED", "checksum divergente: "+entry.Source)
		}
		...
		if _, statErr := os.Lstat(part); statErr == nil {
			rollback()
			return nil, workflowError("MIGRATION_REQUIRED", "staging já existe: "+part)
		}
		...
		if readErr != nil || sha256Bytes(partBytes) != entry.SHA256 {
			_ = os.Remove(part)
			rollback()
			return nil, workflowError("MIGRATION_REQUIRED", "checksum do destino divergiu: "+entry.Target)
		}
		if err := os.Rename(part, target); err != nil {
			rollback()
			return nil, workflowError("MIGRATION_REQUIRED", err.Error())
		}
		copied = append(copied, copyPair{source: source, target: target})
	}
	for _, pair := range copied {
		...
		if sourceErr != nil || targetErr != nil || sha256Bytes(source) != sha256Bytes(target) {
			rollback()
			return nil, workflowError("MIGRATION_REQUIRED", "verificação final divergiu: "+pair.target)
		}
	}
	for _, pair := range copied {
	...
	if documentErr != nil {
		rollback()
		return nil, workflowError("MIGRATION_REQUIRED", documentErr.Error())
	}
...
func migrationKnownFiles(directory string) ([]string, error) {
...
func migrationDesignFiles(directory string) ([]string, error) {
	...
	for _, entry := range entries {
		...
		known, walkErr := migrationKnownFiles(path)
		if walkErr != nil {
			return nil, walkErr
		}
