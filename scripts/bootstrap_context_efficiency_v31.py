#!/usr/bin/env python3
"""Integra a camada Context Efficiency v3.1 em uma árvore v3.0."""
...
def replace_once(path: Path, old: str, new: str) -> None:
    ...
    if content.count(old) != 1:
        raise RuntimeError(f"{path}: marcador inesperado para patch: {old[:100]!r}")
...
def patch_readme() -> None:
    path = ROOT / "README.md"
    replace_once(path, "# Bianchini Method v3.0 — Planning Stability", "# Bianchini Method v3.1 — Context Efficiency")
    replace_once(path, "`v3.0` é a versão do pacote.", "`v3.1` é a versão do pacote.")
    replace_once(
        path,
    ...
    if section not in content:
        if marker not in content:
            raise RuntimeError("README: marcador ausente")
...
def insert_section(path: Path, marker: str, section: str) -> None:
    content = path.read_text(encoding="utf-8")
    if section in content:
        return
    if marker not in content:
        raise RuntimeError(f"{path}: marcador ausente")
    path.write_text(content.replace(marker, section + marker, 1), encoding="utf-8")
...
def main() -> int:
    ...
    patch_changelog()
    patch_docs()
    print("Context Efficiency v3.1 integrada.")
    return 0
