...
- **Artefatos externos verificados de fato:** toda tag de imagem Docker, pacote (npm/pub/pip) e asset fixados em spec ou plano devem ter a EXISTÊNCIA e o conteúdo relevante verificados durante o planejamento (`docker manifest inspect <imagem:tag>`, `npm view <pkg>`, `dart pub info`, etc.). Uma tag inexistente ou um pacote sem os arquivos assumidos custa ciclos de correção na execução.
...
- Delegar a subagentes em paralelo (Opus nas complexas, Sonnet nas demais), cada prompt: ler central (vinculante) + seções do plano mestre + inventário; seções obrigatórias (objetivo, não objetivos, requisitos por ID, fluxos, estados, modelo de dados, contratos, erros, segurança, observabilidade, acessibilidade, offline, design, testes, critérios de aceite verificáveis, dependências, riscos, decisões explícitas D-NN-XX, perguntas bloqueadoras) — seções não aplicáveis ao subsistema viram UMA linha agrupada "Não aplicáveis: X, Y, Z", sem seção própria; zero TBD/TODO; self-review inline. Subagentes NÃO executam git (evita corrida de índice); o orquestrador commita após revisar.
...
**Regra de granularidade (obrigatória):** tarefa = menor unidade que um revisor poderia REJEITAR sozinha aprovando as vizinhas. Se duas tarefas consecutivas nunca seriam rejeitadas separadamente, fundir em uma. Alvo: **15 a 30 tarefas por plano**; acima de 35, o agente autor deve fundir ou justificar por escrito no cabeçalho do plano. UI puramente visual com teste golden/screenshot: 1 tarefa por tela ou componente, não por estado ou variação. Setup/configuração/documentação que só existe para servir uma entrega: dobrar para dentro da tarefa da entrega, nunca tarefa própria. O self-review de cada plano inclui verificação explícita desta regra.
...
3. Criar `docs/superpowers/vN/PLANNING_REVIEW.md`: fontes lidas; specs e planos criados (com métricas); cobertura N/N; conflitos resolvidos; decisões pendentes de ratificação (todas as D-NN-XX novas em bloco único); bloqueios externos reais; riscos; resultado dos scans; revisão final enxuta: resultados dos scans (cobertura, placeholders, consistência de tipos, abstrações proibidas) + até 8 perguntas de JULGAMENTO específicas do domínio que scans não respondem (ex.: autoridade do servidor, idempotência offline, fidelidade de design) — sem re-declarar o que os scans já provaram; confirmação de que nenhuma implementação foi iniciada.
