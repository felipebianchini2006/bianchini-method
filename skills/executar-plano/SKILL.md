...
3. **Por tarefa (ou lote):**
   ...
   - Despachar subagente novo (modelo conforme peso) com **digest de contexto**: a(s) tarefa(s) completa(s) do plano (Files, Interfaces, steps), as Global Constraints e os trechos de spec citados pela tarefa — NÃO mandar o subagente reler specs inteiras; a spec fica como referência para consulta pontual. Instrução de seguir TDD à risca (RED observado → mínimo → GREEN → refatorar) e retornar diff + saída dos comandos executados.
